...
#   - litgen checked out at nbsymengine/external/litgen/ (imported via PYTHONPATH)
#   - Python 3.13+
#
...
LITGEN_SRC=${SRC}/external/litgen/src
...
LITGEN_DESCRIBE=$(git -C "$SRC/external/litgen" describe --tags 2>/dev/null || echo "unknown")
LITGEN_SHA=$(git -C "$SRC/external/litgen" rev-parse HEAD 2>/dev/null || echo "unknown")
